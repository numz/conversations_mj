...
async def legifrance_search_conventions(
    ...
    Args:
        ...
        etat_texte: Etat du texte (VIGUEUR par defaut).
        ...
        if fond == FOND_ACCO:
            if date:
                try:
                    ...
                    filtres.append(
                        SearchFilter(facette=FACET_DATE_SIGNATURE, singleDate=ts_sig)
                    )
                except ValueError:
            ...
            if etat_texte:
                filtres.append(
                    SearchFilter(facette=FACET_LEGAL_STATUS, valeurs=[etat_texte])
                )
            if idcc:
                filtres.append(SearchFilter(facette=FACET_IDCC, valeurs=[idcc]))
            if date:
