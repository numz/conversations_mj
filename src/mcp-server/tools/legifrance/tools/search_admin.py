...
async def legifrance_search_admin(
    ...
    Args:
        ...
        date: Date de publication/deliberation (YYYY-MM-DD).
        nor: Numero NOR du document.
        nature_delib: Nature de la deliberation (pour CNIL).
    ...
    try:
        ...
        if fond == FOND_CNIL:
            if nature_delib:
                filtres.append(
                    SearchFilter(facette=FACET_NATURE_DELIB, valeurs=[nature_delib])
                )
            if nor:
            ...
            if date:
                try:
                    ...
                    filtres.append(
                        SearchFilter(facette=FACET_DATE_DELIB, singleDate=ts_del)
                    )
                except ValueError:
        ...
        for raw_item in raw_results:
            ...
            for r in flat_items:
                ...
                if r.id:
                    ...
                    source_tuple = build_source_with_title(
                        r.id, fond, r.title, r.num, r.etat,
                        context=context, raw_data=r.raw
                    )
                    if source_tuple:
                        url, title = source_tuple
