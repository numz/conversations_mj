...
class LegifranceDocument:
    ...
    def from_raw(
        ...
        if "article" in data and data["article"] is None:
            raise LegifranceDocumentNotFoundError(
                document_id, "API returned null article"
            )
    ...
    def _extract_text(cls, article: dict[str, Any]) -> str:
        ...
        if not text_content:
            if "sections" in article or "articles" in article:
                ...
                text_content = (
                    f"[Structure JORF]\n"
                    f"Eléments: {list_count}\n"
                    f"(Ce document est un sommaire JORF.)"
                )
    ...
    def _build_structure_summary(cls, article: dict[str, Any]) -> str:
        ...
        summary_parts = [
            f"[Structure Document - {article.get('nature', DEFAULT_NATURE_FALLBACK)}]"
        ]
