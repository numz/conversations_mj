...
class LegifranceAPI:
    ...
    def __init__(self) -> None:
        ...
        self.base_url = os.environ.get("LEGIFRANCE_API_BASE_URL", DEFAULT_API_BASE_URL)
        self.oauth_url = os.environ.get("LEGIFRANCE_OAUTH_URL", DEFAULT_OAUTH_URL)
        ...
        self.oauth_host = os.environ.get("LEGIFRANCE_OAUTH_HOST", "")
        self.api_host = os.environ.get("LEGIFRANCE_API_HOST", "")

        # Try static token first, then check cache, then fetch dynamic one
        self.token = os.environ.get("LEGIFRANCE_API_TOKEN", "")
    ...
    def clean_text(text: Any) -> str:
        ...
        tools.legifrance.core.text_utils.
        """
    ...
    async def _request_with_retry(
        ...
        if "json" in kwargs:
            logger.debug(
                "Legifrance request to %s: %s", url, json.dumps(kwargs["json"], indent=2)
            )
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if resp.status_code < 400:
                    try:
                        logger.debug(
                            "Legifrance response: %s", json.dumps(resp.json(), indent=2)
                        )
                    except Exception:
                        pass
    ...
    async def search(
        ...
        with track_request("search", fond=fond, page=page_number) as ctx:
            ...
            try:
                ...
                log_api_response(
                    "search", ctx.duration_ms, True,
                    result_count=len(results), fond=fond
                )
                return results
            except ValidationError as ve:
                slog.warning(
                    "Search response validation warning (using raw data)",
                    error=str(ve)[:100]
                )
    ...
    async def get_document(
        self, article_id: str, use_cache: bool = True
    ) -> dict[str, Any] | None:
        """
        ...
        with track_request("get_document", article_id=article_id) as ctx:
            ...
            if use_cache:
                ...
                if cached_result is not None:
                    ...
                    ctx.cache_hit = True
                    log_api_response(
                        "get_document", ctx.duration_ms, True,
                        cache_hit=True, article_id=article_id
                    )
            ...
            elif article_id.isdigit() or (
                len(article_id) < 20 and not article_id.isalpha()
            ):
            ...
            try:
                ...
                if not data:
                    log_api_response(
                        "get_document", ctx.duration_ms, True,
                        article_id=article_id, found=False
                    )
                    return None
                ...
                try:
                    ...
                    slog.warning(
                        "Document response validation warning (using raw data)",
                        error=str(ve)[:100]
                    )
                ...
                log_api_response(
                    "get_document", ctx.duration_ms, True,
                    article_id=article_id, found=True
                )
                return data
    ...
    async def get_article_with_id_and_num(
        self, text_id: str, num: str
    ) -> dict[str, Any] | None:
        """
    ...
    async def search_code_article(
        ...
        with track_request(
            "search_code_article", fond=FOND_CODE_DATE, query=query
        ) as ctx:
            # Check cache first
            if use_cache:
                criteres = [{"code_name": code_name, "article_num": article_num}]
                cache_key = get_search_cache_key(
                    FOND_CODE_DATE, criteres, [], SORT_PERTINENCE, 1
                )
                cached_result = get_cached(cache_key)
                if cached_result is not None:
                    CacheStats.record_hit()
                    ctx.cache_hit = True
                    log_api_response(
                        "search_code_article", ctx.duration_ms, True,
                        result_count=len(cached_result), cache_hit=True
                    )
                    return cached_result
                CacheStats.record_miss()
            ...
            if not data:
                log_api_response(
                    "search_code_article", ctx.duration_ms, True, result_count=0
                )
                return []
            ...
            try:
                ...
                slog.warning(
                    "Code article search validation warning (using raw data)",
                    error=str(ve)[:100]
                )
                raw_results = data.get("results")
    ...
    async def list_codes(
        ...
        with track_request("list_codes", query=code_name or "(all)") as ctx:
            ...
            if cached_result is not None:
                ...
                ctx.cache_hit = True
                log_api_response(
                    "list_codes", ctx.duration_ms, True,
                    result_count=len(cached_result), cache_hit=True
                )
                return cached_result
            ...
            data = await self._execute_request(
                "post", ENDPOINTS["list_code_legacy"], payload
            )
            if not data:
            ...
            try:
                ...
                slog.warning(
                    "Code list response validation warning (using raw data)",
                    error=str(ve)[:100]
                )
                # Fall back to raw data if validation fails
                raw_results = data.get("results")
            ...
            log_api_response(
                "list_codes", ctx.duration_ms, True, result_count=len(result)
            )
            return result
