...
def validate_date_format(v: str | None) -> str | None:
    ...
    if not DATE_PATTERN.match(v):
        raise ValueError(f"Date invalide '{v}'. Format attendu: YYYY-MM-DD (ex: 2024-01-15)")
...
class SearchConventionsInput(BaseModel):
    ...
    def validate_idcc(cls, v: str | None) -> str | None:
        ...
        if v and not v.isdigit():
            raise ValueError(f"IDCC invalide '{v}'. L'IDCC doit être un nombre (ex: 1090)")
        return v
...
class SearchAdminInput(BaseModel):
    ...
    def validate_nor(cls, v: str | None) -> str | None:
        ...
        if v and len(v) != 12:
            raise ValueError(
                f"Format NOR invalide '{v}'. Le NOR doit contenir 12 caractères (ex: ECOX2300001A)"
            )
        return v
...
class GetDocumentInput(BaseModel):
    ...
    article_id: Annotated[str, Field(min_length=1, description="Identifiant du document")]
...
class SearchCodeArticleInput(BaseModel):
    ...
    code_name: Annotated[str, Field(min_length=1, description="Nom du code (ex: Code pénal)")]
    article_num: Annotated[
        str, Field(min_length=1, description="Numéro d'article (ex: 1240, L123-1)")
    ]
