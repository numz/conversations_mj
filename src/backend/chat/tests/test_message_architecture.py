...
class TestGetComputedMessages:
    ...
    def test_feedback_enrichment_when_flag_on(self):
        pydantic_json = _make_pydantic_json([("user", "Hello"), ("assistant", "Hi!")])
        conv = factories.ChatConversationFactory(pydantic_messages=pydantic_json)
    ...
    def test_feedback_not_enriched_when_flag_off(self):
        pydantic_json = _make_pydantic_json([("user", "Hello"), ("assistant", "Hi!")])
        conv = factories.ChatConversationFactory(pydantic_messages=pydantic_json)
    ...
    @override_settings(EXTENDED_METRICS_ENABLED=True)
    def test_usage_enrichment_when_flag_on(self):
        pydantic_json = _make_pydantic_json([("user", "Hello"), ("assistant", "Hi!")])
        conv = factories.ChatConversationFactory(pydantic_messages=pydantic_json)
    ...
    @override_settings(EXTENDED_METRICS_ENABLED=False)
    def test_usage_not_enriched_when_flag_off(self):
        pydantic_json = _make_pydantic_json([("user", "Hello"), ("assistant", "Hi!")])
        conv = factories.ChatConversationFactory(pydantic_messages=pydantic_json)
    ...
    def test_sources_enrichment(self):
        """Sources are always enriched (Feature 22's own data)."""
        pydantic_json = _make_pydantic_json([("user", "Hello"), ("assistant", "Hi!")])
        conv = factories.ChatConversationFactory(pydantic_messages=pydantic_json)
...
class TestSerializerConditional:
    ...
    @override_settings(MESSAGE_ARCHITECTURE_ENABLED=True, LOCAL_FEEDBACK_ENABLED=True)
    def test_flag_on_includes_feedback_when_feedback_flag_on(self):
        pydantic_json = _make_pydantic_json([("user", "Hello"), ("assistant", "Hi!")])
        conv = factories.ChatConversationFactory(pydantic_messages=pydantic_json)
    ...
    @override_settings(MESSAGE_ARCHITECTURE_ENABLED=True, LOCAL_FEEDBACK_ENABLED=False)
    def test_flag_on_no_feedback_when_feedback_flag_off(self):
        pydantic_json = _make_pydantic_json([("user", "Hello"), ("assistant", "Hi!")])
        conv = factories.ChatConversationFactory(pydantic_messages=pydantic_json)
        ...
        messages = serializer.data["messages"]
        assistant_msg = [m for m in messages if m["role"] == "assistant"][0]
        assert assistant_msg.get("feedback") is None
